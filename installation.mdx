...
async function test() {
  const og = await OpenGeometry.create({
    wasmURL: '/path/to/opengeometry_bg.wasm' // optional in development
  });
  
  console.log('OpenGeometry version:', OpenGeometry.version);
  console.log('Vector3 test:', new Vector3(1, 2, 3));
}
...
<Note>
  The `wasmURL` parameter is optional during development. For production deployments, specify the path where your bundler outputs the WASM file.
</Note>
